...
- `home-mockup-reference.html` was not available for this mission. `index.astro` and its component breakdown (Hero, MusicTeaser, LasCintasPreview, NotasDeCintaTeaser, SintonizaTeaser) were built directly from this document's section descriptions instead of ported from an approved mockup — this is a **deviation from the acceptance criterion "Home page visually matches the approved mockup's structure and copy"**, since no mockup existed to match. Treat the home page as a first-pass structural draft pending either the real mockup or explicit sign-off on this draft.
- The brand manual referenced by Esteban was not attached to this mission; the "four-color stripe device" color selection in `Stripe.astro` is a placeholder pick (4 of the 6 tokens) pending that manual, flagged in the README.
