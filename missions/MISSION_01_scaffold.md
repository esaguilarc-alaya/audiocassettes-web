...
- **Not built, flagged as a gap for a future mission:** the manual's isologo (concentric-circle cassette-reel icon inside the wordmark's "o", manual p.7–9). The header currently uses a plain-text Fredoka wordmark. This needs an actual SVG asset, not a CSS-only fix — reasonable to scope as its own follow-up rather than folding into this one.
